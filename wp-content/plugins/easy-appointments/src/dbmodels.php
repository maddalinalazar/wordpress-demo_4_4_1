...
class EADBModels
{
	...
	public function replace( $table_name, $data, $json = false, $forceStrings = false )
	{
		...
		foreach ($data as $key => $value) {
			...
			if(strlen($value) > 0 && substr($value, 0, 1) == '0') {
				...
				$types[] = ( is_numeric($value) && !$forceStrings ) ? '%d' : '%s';
			}
		}
	...
	public static function delete_reservations()
	{
		global $wpdb;

		$table_app = $wpdb->prefix . 'ea_appointments';

		$query = "DELETE FROM $table_app WHERE status = 'reservation' AND created < (NOW() - INTERVAL 50 MINUTE)";
